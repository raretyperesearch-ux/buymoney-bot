...
// ─── Also register on any group command (catches groups added before this update) ───
...
async function announceWin(roundNumber, winnerName, winnerBag, playerCount, totalPot) {
  ...
  for (const chatId of activeGroups) {
    try {
      ...
      if (err.message?.includes("kicked") || err.message?.includes("blocked") || err.message?.includes("not a member") || err.message?.includes("chat not found")) {
